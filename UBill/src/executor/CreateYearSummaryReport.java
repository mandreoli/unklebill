...
public class CreateYearSummaryReport {
	...
	public CreateYearSummaryReport(String filename, String year) {
		this.year = year;
		...
		Paragraph p1 = new Paragraph(new Chunk("Summary of "+year, title));
	...
	private PdfPTable generateTable() {
		...
		PdfPCell c1 = new PdfPCell(new Paragraph(new Chunk("Month", header)));
		...
		for (int month = 1; month <= Month.values().length; month++) {
			...
			exitTransf = 0.0;
			total = 0.0;
			ts = Transactions.loadTransactions(Login.getUser().getUser(), Login.getAccount().getAccount(), Integer.valueOf(year), month);
			...
			c1.setPhrase(new Paragraph(new Chunk(Date.getMonth(month), data)));
