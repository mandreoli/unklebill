...
public class CreateReport {
	...
	public CreateReport(String filename, String month, String year) {
		...
		PdfPCell c1 = new PdfPCell(new Paragraph(new Chunk("Day", subtitle)));
		PdfPCell c2 = new PdfPCell(new PdfPCell(new Paragraph(new Chunk("Entrance", subtitle))));		
		PdfPCell c3 = new PdfPCell(new Paragraph(new Chunk("Exit", subtitle)));
		PdfPCell c4 = new PdfPCell(new Paragraph(new Chunk("Category", subtitle)));
		...
		for (Transaction t : Transactions.loadTransactions(Login.getUser().getUser(), Login.getAccount().getAccount(), Integer.valueOf(year), Date.getMonth(month)).getTransactions()) {
			if (t.getRefid() == 0) {
				if (t.getType() == '+') {
					...
					c2.setPhrase(new Paragraph(new Chunk(String.valueOf(t.getPayment()), dataActive)));
					...
					c3.setPhrase(new Paragraph(new Chunk(String.valueOf(t.getPayment()), dataPassive)));
				...
				if (t.getType() == '+') {
					...
					c2.setPhrase(new Paragraph(new Chunk(String.valueOf(t.getPayment()), dataNeutro)));
					...
					c3.setPhrase(new Paragraph(new Chunk(String.valueOf(t.getPayment()), dataNeutro)));
